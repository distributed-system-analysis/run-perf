...
node(worker_node) {
    stage('Preprocess') {
        // Checkout upstream qemu
        if (! upstream_qemu_good ) {
            error "Sha of the GOOD qemu commit not specified"
        }
        if (! upstream_qemu_bad ) {
            error "Sha of the BAD qemu commit not specified"
        }
        sh 'dnf install --skip-broken -y python3-devel zlib-devel gtk3-devel glib2-static spice-server-devel usbredir-devel make gcc libseccomp-devel numactl-devel libaio-devel git ninja-build'
