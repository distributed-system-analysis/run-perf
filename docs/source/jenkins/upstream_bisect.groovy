...
node(worker_node) {
    ...
    stage('Measure') {
        ...
        lock(machine) {
            ...
            sh '$KINIT'
            sh "DIFFPERF='python3 scripts/diff-perf' contrib/upstream_qemu_bisect.sh upstream_qemu/ ${upstream_qemu_good} ${upstream_qemu_bad} python3 scripts/run-perf ${extra_args} -vvv --hosts ${machine} --distro ${distro} --provisioner Beaker --default-password QwAo2U6GRxyNPKiZaOCx redhat --profiles ${profiles} --paths ./downstream_config --metadata 'build=${currentBuild.number}${description_prefix}' 'url=${currentBuild.absoluteUrl}' 'project=virt-perf-ci ${currentBuild.projectName}' 'pbench_server=pbench.perf.lab.eng.bos.redhat.com' 'machine_url_base=https://beaker.engineering.redhat.com/view/%(machine)s' ${metadata} -- ${tests}"
        }
    }
